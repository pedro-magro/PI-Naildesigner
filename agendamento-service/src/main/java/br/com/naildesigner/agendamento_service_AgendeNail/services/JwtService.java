...
public class JwtService {
 ...
 // Extrai o userId da claim "userId" do token.
    public UUID extractUserId(String token) {
        String userIdAsString = extractClaim(token, claims -> claims.get("userId", String.class));
        if (userIdAsString != null) {
            return UUID.fromString(userIdAsString);
        }
        throw new IllegalArgumentException("Claim 'userId' não encontrada ou nula no token JWT.");
    }
